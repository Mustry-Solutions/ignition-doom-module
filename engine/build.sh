...
#
# The results (websockets-doom.{js,wasm} and websockets-heretic.{js,wasm}) are
...
# doom-wasm dropped Chocolate Doom's other games, so src/heretic/ is taken from
...
# build and ports the same three game-side changes to it.
#
# Usage:
...
git -C "${WORK}" checkout --quiet "${CHOCOLATE_BASE}" -- src/heretic
...
echo "Engines rebuilt from doom-wasm ${UPSTREAM_COMMIT} + chocolate-doom ${CHOCOLATE_BASE} (heretic)."
