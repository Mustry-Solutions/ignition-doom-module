...
`gateway/.../doom/default.cfg`, `heretic/heretic.cfg`, `hexen/hexen.cfg` and `strife/strife.cfg`
...
http://localhost:9188/data/perspective/client/verify (the launcher; the Doom
control room is `/doom/control-room`), click the game and see E1M1 render.
Check the browser console is clean. The launcher and the per-game hubs are
generated by `ops/verify/tools/build_launcher.py` (edit the `GAMES` table,
rerun, commit the JSON); the other views are hand-authored JSON.
